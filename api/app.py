...
# =============================================================================
# IPL 2026 Squads Endpoint
# =============================================================================

# All 70 IPL 2026 match IDs (discovered April 2026, series ID 9241)
IPL_2026_MATCH_IDS = [
    149618, 149629, 149640, 149651, 149662, 149673, 149684, 149695,
    149699, 149710, 149721, 149732, 149743, 149746, 149757, 149768,
    149779, 149790, 149801, 149812, 151752, 151763, 151774, 151785,
    151796, 151807, 151818, 151829, 151840, 151845, 151856, 151867,
    151878, 151889, 151891, 151902, 151913, 151924, 151935, 151943,
    151954, 151965, 151976, 151987, 151998, 152009, 152020, 152031,
    152042, 152053, 152064, 152075, 152086, 152097, 152108, 152119,
    152130, 152141, 152152, 152163, 152174, 152185, 152196, 152207,
    152218, 152229, 152240, 152241, 152252, 152263,
]

# Team IDs for all 10 IPL 2026 franchises
IPL_2026_TEAM_IDS = frozenset([58, 59, 61, 62, 63, 64, 65, 255, 966, 971])

_ipl2026_squads_cache = None


_PLAYER_ROLES = frozenset([
    'Batter', 'WK-Batter', 'Batting Allrounder', 'Bowling Allrounder',
    'Bowler', 'Allrounder', 'All-Rounder',
])


def _extract_players_from_match_squads_page(text):
    '''
    Decode all self.__next_f.push RSC chunks from a cricket-match-squads page
    and extract player objects using the profileUrl field as an anchor.
    Only includes entries with cricket player roles (excludes coaching staff).
    Returns a dict keyed by player id string.
    '''
    pos = 0
    chunks = []
    while True:
        idx = text.find('self.__next_f.push', pos)
        if idx == -1:
            break
        chunk_start = text.find('[1,"', idx)
        if chunk_start == -1:
            pos = idx + 1
            continue
        chunk_start += 4
        chunk_end = text.find('"]\n', chunk_start)
        if chunk_end == -1:
            chunk_end = text.find('"])', chunk_start)
        if chunk_end == -1:
            pos = idx + 1
            continue
    ...
    for m in re.finditer(r'"profileUrl"\s*:\s*"(/profiles/(\d+)/([^"]+))"', full):
        profile_url = m.group(1)
        slug = m.group(3)
        # Look back 600 chars for sibling fields in the same player object
        ctx = full[max(0, m.start() - 600):m.end()]
        id_m = re.search(r'"id"\s*:\s*(\d+)', ctx)
        name_m = re.search(r'"name"\s*:\s*"([^"]+)"', ctx)
        team_m = re.search(r'"teamId"\s*:\s*(\d+).*?"teamName"\s*:\s*"([^"]*)"', ctx, re.DOTALL)
        role_m = re.search(r'"role"\s*:\s*"([^"]*)"', ctx)
        overseas_m = re.search(r'"isOverseas"\s*:\s*(true|false)', ctx)
        if not (id_m and name_m):
            continue
        role = role_m.group(1) if role_m else ''
        # Skip coaching/support staff
        if role and role not in _PLAYER_ROLES:
            continue
        pid = id_m.group(1)
        players[pid] = {
            'id': int(pid),
            'name': name_m.group(1),
            'role': role,
            'teamId': int(team_m.group(1)) if team_m else 0,
            'teamName': team_m.group(2) if team_m else '',
            'profileUrl': profile_url,
            'slug': slug,
            'isOverseas': overseas_m.group(1) == 'true' if overseas_m else False,
        }

    return players


@app.route('/v2/ipl2026_squads')
def v2_get_ipl2026_squads():
    '''
    Fetch all IPL 2026 player squads by scraping match squad pages on Cricbuzz.
    Iterates through IPL 2026 match IDs until all 10 teams are covered, then
    stops. Results are cached in memory; pass ?refresh=1 to force a refresh.

    Returns JSON: { seriesId, seriesName, teamsCount, totalPlayers, teams: [{
        teamId, teamName, players: [{ id, name, role, slug, profileUrl, isOverseas }]
    }] }
    '''
    global _ipl2026_squads_cache

    force_refresh = request.args.get('refresh', '').lower() in ('1', 'true')

    if _ipl2026_squads_cache is not None and not force_refresh:
        return _ipl2026_squads_cache

    all_players = {}   # pid (str) -> player dict
    teams_covered = set()

    for match_id in IPL_2026_MATCH_IDS:
        if teams_covered >= IPL_2026_TEAM_IDS:
            break
        try:
            url = f'https://www.cricbuzz.com/cricket-match-squads/{match_id}'
            r = requests.get(url, headers=V2_HEADERS, timeout=10)
            if r.status_code != 200:
                continue
            players = _extract_players_from_match_squads_page(r.text)
            if not players:
                continue
            for pid, player in players.items():
                if pid not in all_players:
                    all_players[pid] = player
                teams_covered.add(player['teamId'])
        except Exception:
            continue
